...
public class OperacionProvider(IConfiguration cfg):IOperacionProviderDTO {
 ...
 public Task<List<MovimientoCajaDTO>> MovimientosCaja(int user,int branch)=>List<MovimientoCajaDTO>("dbo.sp_movimientos_caja",new{IdUsuario=user,IdSucursal=branch});
 ...
 public async Task<int> CrearCliente(ClienteGestionDTO client){await using var connection=new SqlConnection(cs);return await connection.QuerySingleAsync<int>("dbo.sp_crear_cliente",new{client.Nombre,client.Nit,client.Telefono,client.Direccion},commandType:CommandType.StoredProcedure);}
