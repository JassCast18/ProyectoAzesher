    ...
    public static byte[] AbonoPdf(AbonoReciboDTO item)
    {
        ...
        return Document.Create(document=>document.Page(page=>
        {
            ...
            page.Content().Border(1.5f).BorderColor("#183B56").Padding(22).Column(column=>
            {
                ...
                column.Item().Border(1).BorderColor(Colors.Grey.Lighten2).Background("#F8FAFC").Padding(14).Column(info=>{info.Item().Text(item.Cliente).Bold().FontSize(14);info.Item().Text($"NIT: {item.Nit??"CF"}");if(!string.IsNullOrWhiteSpace(item.Telefono))info.Item().Text($"Teléfono: {item.Telefono}");if(!string.IsNullOrWhiteSpace(item.Correo))info.Item().Text($"Correo: {item.Correo}");info.Item().Text($"Sucursal: {item.Sucursal}");info.Item().Text($"Nota de crédito relacionada: {item.NotaCredito}").Bold();});
