...
BEGIN
    ...
    WHERE cc.activo=1 AND (cc.fecha_vencimiento_autorizacion IS NULL OR cc.fecha_vencimiento_autorizacion>=CAST(GETDATE() AS DATE))
...
BEGIN
 SET NOCOUNT ON;
 SELECT cx.id_cuenta IdCuenta,c.id_cliente IdCliente,c.nombre Cliente,c.nit Nit,r.id_recibo IdReciboOrigen,r.numero_recibo NotaCredito,
 ...
 GROUP BY cx.id_cuenta,c.id_cliente,c.nombre,c.nit,r.id_recibo,r.numero_recibo,v.fecha,v.total,cx.monto_inicial,cx.saldo_pendiente,cx.numero_cuotas
...
BEGIN
 ...
 WHERE c.id_cliente=@IdCliente GROUP BY c.id_cliente,c.nombre,c.nit,c.telefono,cc.limite_credito;
 ...
 FROM(
   SELECT -q.id_cuota IdMovimiento,q.id_cuenta IdCuenta,CAST(q.fecha_vencimiento AS DATETIME) Fecha,'Cuota' Tipo,
          CONCAT('Cuota ',q.numero_cuota) Documento,q.monto Debito,q.monto_pagado Credito,q.estado Estado,q.fecha_vencimiento FechaVencimiento
   FROM dbo.cuota_cuenta_cobrar q JOIN dbo.cuenta_cobrar cx ON cx.id_cuenta=q.id_cuenta JOIN dbo.venta v ON v.id_venta=cx.id_venta
   JOIN dbo.factura f ON f.id_venta=v.id_venta JOIN dbo.recibo r ON r.id_factura=f.id_factura
   WHERE v.id_cliente=@IdCliente AND r.id_sucursal=@IdSucursal AND cx.estado<>'Anulada'
   UNION ALL
...
CREATE OR ALTER PROCEDURE dbo.sp_registrar_abono_core
 @IdCuenta INT,@IdSucursal INT,@IdUsuario INT,@Monto DECIMAL(12,2),@MetodoPago VARCHAR(30),@Referencia VARCHAR(100)=NULL
AS
BEGIN
 ...
 BEGIN TRY
  ...
  END;
  INSERT dbo.abono(id_cuenta,fecha,monto,numero_abono,metodo_pago,referencia,saldo_anterior,saldo_posterior,id_recibo_origen,id_sucursal,id_sesion,id_usuario)
  VALUES(@IdCuenta,GETDATE(),@Monto,'PENDIENTE',@MetodoPago,NULLIF(LTRIM(RTRIM(@Referencia)),''),@Saldo,@Saldo-@Monto,@IdRecibo,@IdSucursal,@IdSesion,@IdUsuario);
...
BEGIN
 ...
 SELECT a.id_abono IdAbono,a.numero_abono NumeroAbono,a.fecha,a.monto,a.metodo_pago MetodoPago,a.referencia,
        ...
        s.nombre Sucursal,u.nombre Usuario,cx.id_cuenta IdCuenta,v.total TotalCredito,
...
BEGIN
 ...
 IF EXISTS(SELECT 1 FROM dbo.cliente_credito WHERE id_cliente=@IdCliente)
  UPDATE dbo.cliente_credito SET limite_credito=@LimiteCredito,dias_maximos_pago=@DiasMaximosPago,fecha_vencimiento_autorizacion=@FechaVencimientoAutorizacion,observaciones=NULLIF(LTRIM(RTRIM(@Observaciones)),''),activo=@Activo,fecha_autorizacion=GETDATE(),id_usuario_autorizo=@IdUsuario WHERE id_cliente=@IdCliente;
 ELSE
  INSERT dbo.cliente_credito(id_cliente,limite_credito,dias_maximos_pago,fecha_vencimiento_autorizacion,observaciones,activo,id_usuario_autorizo) VALUES(@IdCliente,@LimiteCredito,@DiasMaximosPago,@FechaVencimientoAutorizacion,NULLIF(LTRIM(RTRIM(@Observaciones)),''),@Activo,@IdUsuario);
