...
export default function MainLayout({ children }) {
  ...
  useEffect(() => {
    ...
    if (saleFlow(previousPath.current) && !saleFlow(location.pathname)) clearSalesDraft();
  ...
  const [isInventariosOpen, setIsInventariosOpen] = useState(() =>
    location.pathname.startsWith("/inventarios"),
  );
  const [isCajaOpen, setIsCajaOpen] = useState(() =>
    location.pathname.startsWith("/caja"),
  );
  const [isClientesOpen, setIsClientesOpen] = useState(() =>
    location.pathname.startsWith("/clientes"),
  );
  const [isReportesOpen, setIsReportesOpen] = useState(() =>
    location.pathname.startsWith("/reportes"),
  );
  const [isCobrosOpen, setIsCobrosOpen] = useState(() =>
    location.pathname.startsWith("/cobros"),
  );
  const [isTrabajadoresOpen, setIsTrabajadoresOpen] = useState(() =>
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto whitespace-nowrap">
          ...
          {hasModuleAccess("inventarios") && (
            ...
            >
              ...
              <ChevronDown
                className={`ml-auto h-4 w-4 transition-transform ${isInventariosOpen ? "rotate-180" : ""}`}
              />
            </button>
          )}
          {hasModuleAccess("inventarios") && isInventariosOpen && (
            <div className="ml-5 space-y-1 border-l border-slate-200 pl-3">
              ...
              <NavLink to="/inventarios/salidas" className={({ isActive }) => `flex items-center gap-2 rounded-lg px-3 py-2 text-sm ${isActive ? "bg-teal-50 font-semibold text-brand-teal" : "text-slate-600 hover:bg-slate-50"}`}><PackageSearch className="h-4 w-4" />Salida de producto</NavLink>
              <NavLink
                to="/inventarios/proveedores"
                className={({ isActive }) =>
                  `flex items-center gap-2 rounded-lg px-3 py-2 text-sm ${isActive ? "bg-teal-50 font-semibold text-brand-teal" : "text-slate-600 hover:bg-slate-50"}`
                }
              >
              ...
              {isAdministrator && (
                <>
                  <NavLink
                    to="/inventarios/traslados/nuevo"
                    className={({ isActive }) =>
                      `flex items-center gap-2 rounded-lg px-3 py-2 text-sm ${isActive ? "bg-teal-50 font-semibold text-brand-teal" : "text-slate-600 hover:bg-slate-50"}`
                    }
                  >
          ...
          {hasModuleAccess("ventas") && (
            <button
              type="button"
              onClick={() => setIsVentasOpen((current) => !current)}
              className={`flex w-full items-center gap-3 rounded-lg px-4 py-3 text-sm font-medium transition-colors ${location.pathname.startsWith("/ventas") ? "bg-brand-teal text-white" : "text-slate-600 hover:bg-slate-50"}`}
            >
              ...
              <ChevronDown
                className={`ml-auto h-4 w-4 transition-transform ${isVentasOpen ? "rotate-180" : ""}`}
              />
            </button>
          )}
          {hasModuleAccess("ventas") && isVentasOpen && (
            <div className="ml-5 space-y-1 border-l border-slate-200 pl-3">
              <NavLink
                to="/ventas"
                end
          ...
          {hasModuleAccess("caja") && (
            <MenuGroup
              ...
              icon={WalletCards}
              open={isCajaOpen}
              active={location.pathname.startsWith("/caja")}
              onClick={() => setIsCajaOpen((value) => !value)}
            >
              <SubLink to="/caja/apertura-cierre" icon={Banknote}>
                Apertura y cierre
              </SubLink>
              <SubLink to="/caja/cierres" icon={History}>
                Listado de cierres
              </SubLink>
            </MenuGroup>
          )}
          {hasModuleAccess("clientes") && (
            <MenuGroup
              ...
              onClick={() => setIsClientesOpen((value) => !value)}
            >
              <SubLink to="/clientes/crear" icon={UserRoundPlus}>
                Crear cliente
              </SubLink>
              <SubLink to="/clientes/listado" icon={History}>
                Listado de clientes
              </SubLink>
            </MenuGroup>
          )}
          {hasModuleAccess("cobros") && (
            <MenuGroup
              label="Cobros"
              icon={HandCoins}
              open={isCobrosOpen}
            ...
            >
              ...
              {isAdministrator && (
                <SubLink to="/cobros/autorizar" icon={ShieldCheck}>
                  Autorizar crédito
                </SubLink>
              )}
            </MenuGroup>
          ...
          {hasModuleAccess("reportes") && (
            <MenuGroup
              ...
              onClick={() => setIsReportesOpen((value) => !value)}
            >
              <SubLink to="/reportes/consolidado" icon={BarChart3}>
                Listado de reportes
              </SubLink>
            </MenuGroup>
          )}
          {hasModuleAccess("trabajadores") && (
            <MenuGroup
              ...
              active={location.pathname.startsWith("/trabajadores")}
              onClick={() => setIsTrabajadoresOpen((value) => !value)}
            >
              {isAdministrator && (
                <SubLink to="/trabajadores/registro" icon={UserRoundPlus}>
                  Registro
                </SubLink>
              )}
              <SubLink to="/trabajadores/kpis" icon={BarChart3}>
                KPIs
              </SubLink>
              {isAdministrator && (
                <SubLink to="/trabajadores/evaluacion" icon={Star}>
                  Evaluar trabajador
                </SubLink>
              )}
          ...
          <div className="flex items-center gap-3 sm:gap-5">
            ...
            <div className="hidden sm:flex relative items-center border border-gray-200 rounded-lg bg-white hover:bg-slate-50 transition-colors focus-within:border-brand-teal focus-within:ring-1 focus-within:ring-brand-teal">
              ...
              <select
                ...
                title={
                  isSucursalLocked
                    ? "Finaliza la venta o vacía el pedido para cambiar de sucursal."
            ...
            <div className="flex items-center gap-3 group cursor-pointer hover:bg-slate-50 p-1.5 rounded-lg transition-colors">
              ...
              <div className="hidden flex-col text-left sm:flex">
                <span className="text-sm font-bold text-slate-800 leading-none">
                  {user?.nombre || user?.unique_name || "Administrador general"}
                </span>
  ...
  return (
    <>
      <button
        type="button"
        onClick={onClick}
        className={`flex w-full items-center gap-3 rounded-lg px-4 py-3 text-sm font-medium transition-colors ${active ? "bg-brand-teal text-white" : "text-slate-600 hover:bg-slate-50"}`}
      >
